...
export const AFFIRMATIONS: Record<AgeGroup, string[]> = {
  "3-6": [
    "Ты хороший(ая) такой(ая), какой(ая) есть.",
    "Тебя любят, что бы ни случилось.",
    "Все чувства можно чувствовать — это нормально.",
    "Ты не один(на), рядом всегда есть кто-то родной.",
  ],
  "7-11": [
    "Ты имеешь право на любые чувства.",
    "Трудности не делают тебя хуже.",
    "Ты справляешься, даже когда трудно.",
    "Просить о помощи — это сила, а не слабость.",
  ],
  "12-17": [
    "Твои чувства обоснованы, даже если кажется иначе.",
    "Ты не обязан(а) справляться со всем в одиночку.",
    "Плохой день — это не приговор на всю жизнь.",
    "Заботиться о себе — не эгоизм.",
  ],
};

export function getAffirmation(ageGroup: AgeGroup, seed: number = new Date().getDate()): string {
  const list = AFFIRMATIONS[ageGroup];
  return list[seed % list.length];
}

export const SELF_CARE_TIPS: Record<AgeGroup, string[]> = {
  "3-6": [
    "Выпей водички и обними кого-нибудь мягкого.",
    "Попрыгай 10 раз — это помогает выпустить эмоции.",
    "Полепи или порисуй пальчиками пару минут.",
  ],
  "7-11": [
    "Попробуй 5 глубоких вдохов перед тем, как что-то решить.",
    "Нарисуй своё настроение — необязательно кому-то показывать.",
    "Выйди на 5 минут на улицу или хотя бы к окну.",
  ],
  "12-17": [
    "Запиши три вещи, за которые благодарен(на) сегодня.",
    "Сделай паузу от телефона на 15 минут.",
    "Выпей воды и подвигайся хотя бы немного.",
  ],
};

export function getSelfCareTip(ageGroup: AgeGroup, seed: number = new Date().getDate()): string {
  const list = SELF_CARE_TIPS[ageGroup];
  return list[seed % list.length];
}
