...
function createAlert(
  ...
  const existing = db
    ...
    .get(childId, type, message);
  if (existing) return;
  db.prepare(
    `INSERT INTO alerts (id, family_id, child_id, type, severity, message, status)
     VALUES (?, ?, ?, ?, ?, ?, 'open')`
  ).run(nanoid(16), familyId, childId, type, severity, message);
}

/** Called immediately after a gatekeeper safety answer is recorded (12-17). */
export function evaluateGatekeeper(familyId: string, childId: string, answer: number) {
  if (answer === 1) {
    createAlert(
      familyId,
      childId,
...
export function evaluateMoodTrend(familyId: string, childId: string) {
  const rows = db
    .prepare(
      `SELECT mood_value FROM checkins WHERE child_id = ? ORDER BY date DESC LIMIT 7`
    )
    .all(childId) as Pick<Checkin, "mood_value">[];
...
export function evaluateDilemmaPattern(familyId: string, childId: string) {
  const rows = db
    .prepare(
      `SELECT pattern FROM dilemma_responses WHERE child_id = ? ORDER BY date DESC LIMIT 5`
    )
    .all(childId) as { pattern: string }[];
...
export function evaluateWeeklyObservation(
  familyId: string,
  childId: string,
  ...
  const recentWeeks = db
    ...
    .all(childId) as { answers_json: string }[];
...
export function evaluateBiweeklyReport(
  familyId: string,
  childId: string,
  ...
  const recent = db
    ...
    .all(childId) as { domain_scores_json: string }[];
