...
export default function CrisisScreen() {
  return (
    <div className="flex-1 flex flex-col items-center justify-center px-6 py-10 text-center">
      ...
      <h2 className="text-xl font-semibold text-[var(--teal-dark)] mb-3">
        Спасибо, что сказал(а) честно
      </h2>
      <p className="text-sm text-[#5c655f] max-w-sm mb-6 leading-relaxed">
      ...
      <div className="w-full max-w-sm flex flex-col gap-3 mb-6">
        {CRISIS_CONTACTS.map((c) => (
          <div key={c.name} className="bg-white border border-[var(--line)] rounded-2xl p-4 text-left">
            <div className="text-lg font-semibold text-[var(--teal-dark)]">{c.phone}</div>
            <div className="text-sm font-medium">{c.name}</div>
            <div className="text-xs text-[#9a9384] mt-1">{c.note}</div>
          </div>
        ))}
      </div>
      <p className="text-xs text-[#9a9384] max-w-sm">
